...
import numpy as np
from env import DelayMatchSample
...
class ActorCriticCogRNN(nn.Module):
    def __init__(self, input_dim, action_dim):
        ...
        #add postprocessor
        self.postprocessor = nn.Sequential(nn.Linear(80*3, 64), nn.ReLU())
        self.actor = nn.Sequential(
            nn.Linear(64, 64),
            nn.Linear(64, action_dim),
            nn.Softmax(dim=-1)
...
if __name__ == "__main__":
    ...
    env = DelayMatchSample(**env_configs)
    ...
    train(env, model, optimizer, num_episodes=70000, gamma=parsed_args.gamma, ckpt_path=model_ckpt_path)
